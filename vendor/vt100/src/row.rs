...
impl Row {
    ...
    // argus patch: shortening a row can cut a wide character's second half
    // off, leaving a wide cell in the last column that the rest of the crate
    // assumes is followed by its continuation (and panics on). Terminals
    // blank such a half character; do the same.
    fn clear_split_wide(&mut self) {
