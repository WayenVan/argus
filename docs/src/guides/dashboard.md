...
| `H` `J` `K` `L` | Tree: focus the pane to the left, below, above or right (agents on the left; preview, title, recap on the right) |
...
| `x` | Kill |
| `c` | Copy the name; in Tree, what the focused pane shows: the screen, title or recap |
| `q` / `Esc` | Quit |
...
Screen previews show the latest visible part of each agent's screen. In Tree, focus the preview and use `↑`/`↓` (`k`/`j`) or `PgUp`/`PgDn` to scroll through the rest of its current screen; there is no history beyond it. Keys other than `Enter` act on the focused pane, so switch back to the agent list to select another agent. When attaching from the dashboard to an agent using the normal screen at the same terminal size, Argus restores its current screen and continues from that output position instead of replaying the recent output history. If the sizes differ, it falls back to the history replay. A standalone `argus attach` also replays that history into the terminal's scrollback.
